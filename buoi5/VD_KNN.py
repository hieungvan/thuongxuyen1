...
def test_model():
    ...
    between_1_and_2 = np.sum((abs(y_test - y_predict) > 1)
                             & (abs(y_test - y_predict) <= 2))
...
def create_gui():
    ...
    # Nút Load Data
    load_button = tk.Button(window, text=" Chọn File", command=load_data)
    ...
    knn_radio = tk.Radiobutton(
        window, text="KNN", variable=algorithm_var, value="KNN")
    lr_radio = tk.Radiobutton(window, text="Linear Regression",
                              variable=algorithm_var, value="Linear Regression")

    knn_radio.grid(row=1, column=1)
    lr_radio.grid(row=1, column=2)

    train_button = tk.Button(window, text="Training",
                             command=lambda: train_model(algorithm_var.get()))
    ...
    predict_button = tk.Button(
        window, text="Predict Performance Index", command=predict_performance)
